 ...
 * Shows Feed Name and Feed URL fields and a Delete option.
...
class RssFeedSettingsActivity final : public UiListActivity {
 ...
 private:
  ...
  // Row storage: at most 3 rows (Name/URL + Delete, see BASE_ITEMS in the
  // .cpp), so a fixed-capacity array avoids any heap allocation for the row
  // list. Labels are set once in the constructor (they never change);
  // buildScreen() only refreshes the value pointers, which already point at
  // editFeed's own fields (no new strings built).
  static constexpr int MAX_MENU_ITEMS = 3;
